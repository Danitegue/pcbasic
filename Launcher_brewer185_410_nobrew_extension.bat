...
set BREWDIR=C:\PCBasic_Brewer_Repo\brw#185\Program410
...
%PYTHON_DIR%\python.exe %PCBASIC_PATH%\run.py --interface=ansi --mount=C:%MOUNT_C%,D:%MOUNT_D% --run=%PROGRAM% --quit=False -f=10 --shell=cmd.exe --scaling=smooth --extension=Brw_extensions_test --logfile=%LOG_DIR%\pcbasic_brewer_log.txt
