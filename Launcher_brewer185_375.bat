...
set BREWDIR=C:\PCBasic_Brewer_Repo\brw#185\Program375
...
%PCBASIC_PATH%\ansipipe-launcher.exe %PYTHON_DIR%\python.exe %PCBASIC_PATH%\run.py --mount=C:%MOUNT_C%,D:%MOUNT_D% --interface=ansi --run=%PROGRAM% --quit=False -f=10 --shell=cmd.exe --scaling=native --com1=PORT:%COM_PORT% --logfile=%LOG_DIR%\pcbasic_brewer_log.txt
