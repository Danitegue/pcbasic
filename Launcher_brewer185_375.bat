...
%PCBASIC_PATH%\ansipipe-launcher.exe %PYTHON_DIR%\python.exe %PCBASIC_PATH%\run.py --interface=pygame --mount=C:%MOUNT_C%,D:%MOUNT_D% --com1=PORT:%COM_PORT% --run=%PROGRAM% --quit=False -f=10 --shell=cmd.exe --scaling=smooth --logfile=%LOG_DIR%\pcbasic_log.txt
